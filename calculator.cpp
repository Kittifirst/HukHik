...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		textfield = CreateWindow("STATIC", "    Please input two numbers", WS_VISIBLE | WS_CHILD | WS_BORDER, 20, 15, 200, 25, hwnd, NULL, NULL, NULL);
	...
	case WM_COMMAND:
		...
		if (LOWORD(wParam) == 1)
		{
			float sumnum = atof(num1) + atof(num2);
			char t[100];
			sprintf(t, "%f", sumnum);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	wc.hbrBackground = CreateSolidBrush(RGB(144, 238, 144));
